<template>
  <v-btn color="primary" tonal @click="wasteCodesOpen = true">
    <v-icon icon="mdi-format-list-bulleted" /> Browse Federal Waste Codes
  </v-btn>

  <v-dialog v-model="wasteCodesOpen" :fullscreen="smAndDown" max-width="1000" scrollable>
    <v-card>
...
<script setup>
  ...
  async function fetchWasteCodes () {
    ...
    try {
      ...
      error.value = error_?.message || 'Failed to load waste codes.'
    } finally {
      loading.value = false
    }
  }
