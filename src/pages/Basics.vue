  ...
  <v-container :class="`mt-12 px-6 pb-6 mb-12 w-${pageWidth}`">
    <section>
      ...
      <p class="text-body-1 mb-2">Electronic manifests have four main requirements. Click each panel to learn more
        about each one.</p>
      <v-expansion-panels class="elevation-2" variant="accordion">
    ...
    <section>
      ...
      <v-row align="stretch">
        <v-col v-for="(card, i) in electronicCards" :key="i" cols="12" md="6">
          <v-card class="fill-height" elevation="2" rounded="xl" variant="tonal">
      ...
      <v-table class="elevation-4" striped="odd">
        <thead>
          <tr>
            ...
            <th v-for="(item, i) in manifestTypeHeadings" :key="i" class="text-center">
              <div>{{ item.text }}</div>
              <v-icon color="success" :icon="item.icon" size="30" />
            </th>
          </tr>
        </thead>
        <tbody>
    ...
    <section>
      ...
      <p class="text-body-1">There are multiple options for performing signatures on electronic manifests. The
        ...
        the
        available options.</p>
      <v-table class="elevation-4 mt-3" striped="odd">
        <thead>
          <tr>
            <th>
              Signature Type
      ...
      <v-alert border="start" border-color="blue accent-4" class="my-2">
        To learn more about the remote signer policy, see <a
          href="https://rcrapublic.epa.gov/files/14956.pdf"
          target="_blank"
        >the Remote Signer Policy
          memo <v-icon icon="mdi-open-in-new" size="18" /> </a>
      </v-alert>
