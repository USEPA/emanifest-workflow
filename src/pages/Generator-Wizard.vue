      ...
      <v-col cols="12" lg="7" md="8">
        <div class="d-flex align-center justify-center flex-column">
          <div class="d-flex align-center">
            <div class="text-h5 mr-2">Generator Wizard</div>
            <tooltip tip-location="generatorWizardHeading" type="info" />
          </div>
        </div>
        <v-sheet class="pa-3">
          <div>
            <div class="d-flex align-center">
              <div class="text-h6 mr-2">Obligations</div>
            </div>

            <v-radio-group v-model="selectedFedHaz" class="mt-4">
              <template #label><span class="text-wrap">Does your site generate federally-regulated RCRA hazardous
                  waste?</span></template>
            ...
            <v-radio-group v-if="selectedFedHaz === 'yes'" v-model="selectedGeneratorMonthly" class="mt-6">
              <template #label><span class="text-wrap">How much waste is generated at your site on a monthly
                  basis?</span></template>
              <v-radio v-for="(option, key) in genRadioButtons" :key="key" :label="option" :value="key" />
            </v-radio-group>
            ...
            <v-alert v-if="selectedFedHaz === 'no'" class="mt-8" type="info" variant="tonal">
              Site is not subject to federal RCRA requirements. Please verify if your site generates any <a
                ...
                target="_blank">
                state-only regulated waste subject to RCRA requirements <v-icon icon="mdi-open-in-new" size="18" /></a>
              and follow
              applicable regulatory requirements.</v-alert>
                ...
                <tbody>
                  <tr v-for="row in requirementRows" :key="row.key">
                    ...
                    <td v-for="cat in categories" :key="cat.key"
                      ...
                      <v-chip :color="genCategories[cat.key][row.key] ? 'success' : 'error'" label
                        :prepend-icon="genCategories[cat.key][row.key] ? 'mdi-check-circle' : 'mdi-close-circle'"
                        size="small" :variant="genCategories[cat.key][row.key] ? 'tonal' : 'outlined'">
          ...
          <div class="mt-8">
            <v-card>
              ...
              <v-form class="mx-6" @submit.prevent="onSearch">
                <v-text-field v-model="epaId" class="epa-id-field" clearable density="comfortable" :error="!!error"
                  :error-messages="error ? [error] : []" label="EPA ID Number" variant="outlined"
                  @keyup.enter="onSearch">
                  <template #append-inner>
                    <v-btn color="primary" :disabled="!epaId || loading" :loading="loading" size="small" variant="flat"
                      @click="onSearch">
                      Search
                    </v-btn>
                  </template>
            ...
            <v-card v-if="result" class="mt-4">
              <v-card-title class="d-flex align-center">
                ...
                <v-chip v-if="result.activeSite" class="ml-2" color="success" label size="small">
                  Active Site
                </v-chip>
              </v-card-title>
              ...
              <v-card-text>
                ...
                <div v-if="activities.length > 0">
                  ...
                  <div class="mt-2">
                    <v-chip v-for="activity in activities" :key="activity" class="mr-2 mb-2" color="blue" label
                      size="small">
                      {{ activity }}
                    </v-chip>
                  </div>
...
import WasteCodes from '../components/WasteCodes.vue'
